...
configureOptsNoDir econfig bco deps isLocal package = concat
    ...
    , maybe [] (\customProgAlexPath            -> ["--with-alex="              ++ toFilePath customProgAlexPath            ]) (configProgAlexPath            config)
    , maybe [] (\customProgArPath              -> ["--with-ar="                ++ toFilePath customProgArPath              ]) (configProgArPath              config)
    , maybe [] (\customProgC2hsPath            -> ["--with-c2hs="              ++ toFilePath customProgC2hsPath            ]) (configProgC2hsPath            config)
    , maybe [] (\customProgCpphsPath           -> ["--with-cpphs="             ++ toFilePath customProgCpphsPath           ]) (configProgCpphsPath           config)
    , maybe [] (\customProgGccPath             -> ["--with-gcc="               ++ toFilePath customProgGccPath             ]) (configProgGccPath             config)
    , maybe [] (\customProgGhcPath             -> ["--with-ghc="               ++ toFilePath customProgGhcPath             ]) (configProgGhcPath             config)
    , maybe [] (\customProgGhcPkgPath          -> ["--with-ghc-pkg="           ++ toFilePath customProgGhcPkgPath          ]) (configProgGhcPkgPath          config)
    , maybe [] (\customProgGhcjsPath           -> ["--with-ghcjs="             ++ toFilePath customProgGhcjsPath           ]) (configProgGhcjsPath           config)
    , maybe [] (\customProgGhcjsPkgPath        -> ["--with-ghcjs-pkg="         ++ toFilePath customProgGhcjsPkgPath        ]) (configProgGhcjsPkgPath        config)
    , maybe [] (\customProgGreencardPath       -> ["--with-greencard="         ++ toFilePath customProgGreencardPath       ]) (configProgGreencardPath       config)
    , maybe [] (\customProgHaddockPath         -> ["--with-haddock="           ++ toFilePath customProgHaddockPath         ]) (configProgHaddockPath         config)
    , maybe [] (\customProgHappyPath           -> ["--with-happy="             ++ toFilePath customProgHappyPath           ]) (configProgHappyPath           config)
    , maybe [] (\customProgHaskellSuitePath    -> ["--with-haskell-suite="     ++ toFilePath customProgHaskellSuitePath    ]) (configProgHaskellSuitePath    config)
    , maybe [] (\customProgHaskellSuitePkgPath -> ["--with-haskell-suite-pkg=" ++ toFilePath customProgHaskellSuitePkgPath ]) (configProgHaskellSuitePkgPath config)
    , maybe [] (\customProgHmakePath           -> ["--with-hmake="             ++ toFilePath customProgHmakePath           ]) (configProgHmakePath           config)
    , maybe [] (\customProgHpackPath           -> ["--with-hpack="             ++ toFilePath customProgHpackPath           ]) (configProgHpackPath           config)
    , maybe [] (\customProgHpcPath             -> ["--with-hpc="               ++ toFilePath customProgHpcPath             ]) (configProgHpcPath             config)
    , maybe [] (\customProgHsc2hsPath          -> ["--with-hsc2hs="            ++ toFilePath customProgHsc2hsPath          ]) (configProgHsc2hsPath          config)
    , maybe [] (\customProgHscolourPath        -> ["--with-hscolour="          ++ toFilePath customProgHscolourPath        ]) (configProgHscolourPath        config)
    , maybe [] (\customProgLdPath              -> ["--with-ld="                ++ toFilePath customProgLdPath              ]) (configProgLdPath              config)
    , maybe [] (\customProgPkgConfigPath       -> ["--with-pkg-config="        ++ toFilePath customProgPkgConfigPath       ]) (configProgPkgConfigPath       config)
    , maybe [] (\customProgStripPath           -> ["--with-strip="             ++ toFilePath customProgStripPath           ]) (configProgStripPath           config)
    , maybe [] (\customProgTarPath             -> ["--with-tar="               ++ toFilePath customProgTarPath             ]) (configProgTarPath             config)
    , map (("--alex-option="              ++) . T.unpack) (configProgAlexOptions            config)
    , map (("--ar-option="                ++) . T.unpack) (configProgArOptions              config)
    , map (("--c2hs-option="              ++) . T.unpack) (configProgC2hsOptions            config)
    , map (("--cpphs-option="             ++) . T.unpack) (configProgCpphsOptions           config)
    , map (("--gcc-option="               ++) . T.unpack) (configProgGccOptions             config)
    , map (("--ghc-option="               ++) . T.unpack) (configProgGhcOptions             config)
    , map (("--ghc-pkg-option="           ++) . T.unpack) (configProgGhcPkgOptions          config)
    , map (("--ghcjs-option="             ++) . T.unpack) (configProgGhcjsOptions           config)
    , map (("--ghcjs-pkg-option="         ++) . T.unpack) (configProgGhcjsPkgOptions        config)
    , map (("--greencard-option="         ++) . T.unpack) (configProgGreencardOptions       config)
    , map (("--haddock-option="           ++) . T.unpack) (configProgHaddockOptions         config)
    , map (("--happy-option="             ++) . T.unpack) (configProgHappyOptions           config)
    , map (("--haskell-suite-option="     ++) . T.unpack) (configProgHaskellSuiteOptions    config)
    , map (("--haskell-suite-pkg-option=" ++) . T.unpack) (configProgHaskellSuitePkgOptions config)
    , map (("--hmake-option="             ++) . T.unpack) (configProgHmakeOptions           config)
    , map (("--hpack-option="             ++) . T.unpack) (configProgHpackOptions           config)
    , map (("--hpc-option="               ++) . T.unpack) (configProgHpcOptions             config)
    , map (("--hsc2hs-option="            ++) . T.unpack) (configProgHsc2hsOptions          config)
    , map (("--hscolour-option="          ++) . T.unpack) (configProgHscolourOptions        config)
    , map (("--ld-option="                ++) . T.unpack) (configProgLdOptions              config)
    , map (("--pkg-config-option="        ++) . T.unpack) (configProgPkgConfigOptions       config)
    , map (("--strip-option="             ++) . T.unpack) (configProgStripOptions           config)
    , map (("--tar-option="               ++) . T.unpack) (configProgTarOptions             config)
    , ["--ghcjs" | wc == Ghcjs]
