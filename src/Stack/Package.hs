...
hpack pkgDir = do
    ...
    when exists $ do
        ...
        config <- view configL
        case configProgHpackPath config of
            Nothing -> do
            ...
            Just command -> do
                ...
                let opts = map (T.unpack) $ configProgHpackOptions config
                let cmd = Cmd (Just pkgDir) (T.unpack $ pathToText command) envOverride opts
                -- TODO: check that `command` exists and is executable before trying
                runCmd cmd Nothing
